...
async function initiateMikoEmploymentRequest() {
    try {
        ...
        // Add metadata for better linking
        // Metadata addition is not supported, removing the call to addMetadata
...
async function handleEmploymentRequest() {
    try {
        ...
        for (const request of requests.items) {
            ...
            //chwck if the contract is already approved
...
async function initiateBankAccountOpening(identityVc: any, employmentResponseVc: any, registrationResponseVc: any) {
    try {
        ...
        // Get the identity claims for applicant info
        ...
        const bankAccountOpeningDraft = await bankAccountOpeningDecorator.create({
            claims: {
                ...
                // Add required fields for compliance panel
                applicantName: identityClaims.fullName,
                applicantDetails: {
                    fullName: identityClaims.fullName,
                    dateOfBirth: identityClaims.dateOfBirth,
                    nationality: identityClaims.nationality,
                    employmentInfo: {
                        employer: employmentResponseVc.claims.employerName,
                        position: employmentResponseVc.claims.position,
                        salary: employmentResponseVc.claims.salary
                    },
                    registrationInfo: {
                        registrationNumber: registrationResponseVc.claims.registrationNumber,
                        registrationDate: registrationResponseVc.claims.registrationDate
                    }
                },
